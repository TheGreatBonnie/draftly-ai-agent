...
def test_approve_button(mock_resume, mock_complete, mock_verify_token, mock_sig):
    mock_verify_token.return_value = {"reviewer_id": "r1", "review_id": "rev1"}
    payload = _make_interaction(custom_id="discord_approve:tok123")
    resp = client.post(
        "/api/discord/interactions",
        json=payload,
        headers=_headers(),
...
@patch("src.api.routes.discord.complete_review", new_callable=AsyncMock)
@patch("src.api.routes.discord.resume_review", new_callable=AsyncMock)
def test_reject_button(mock_resume, mock_complete, mock_verify_token, mock_sig):
    mock_verify_token.return_value = {"reviewer_id": "r1", "review_id": "rev1"}
    payload = _make_interaction(custom_id="discord_reject:tok123")
    resp = client.post(
        "/api/discord/interactions",
        json=payload,
        headers=_headers(),
...
@patch("src.api.routes.discord.complete_review", new_callable=AsyncMock)
@patch("src.api.routes.discord.resume_review", new_callable=AsyncMock)
def test_revise_button(mock_resume, mock_complete, mock_verify_token, mock_sig):
    mock_verify_token.return_value = {"reviewer_id": "r1", "review_id": "rev1"}
    payload = _make_interaction(custom_id="discord_revise:tok123")
    resp = client.post(
        "/api/discord/interactions",
        json=payload,
        headers=_headers(),
...
@patch("src.api.routes.discord.complete_review", new_callable=AsyncMock)
@patch("src.api.routes.discord.resume_review", new_callable=AsyncMock)
def test_feedback_select(mock_resume, mock_complete, mock_verify_token, mock_sig):
    mock_verify_token.return_value = {"reviewer_id": "r1", "review_id": "rev1"}
    payload = _make_interaction(custom_id="discord_feedback:tok123")
...
@patch("src.api.routes.discord._verify_signature", return_value=True)
@patch("src.api.routes.discord.verify_review_token", return_value=None)
def test_expired_token_returns_error(mock_verify_token, mock_sig):
    payload = _make_interaction(custom_id="discord_approve:expired_token")
    resp = client.post(
        "/api/discord/interactions",
        json=payload,
        headers=_headers(),
